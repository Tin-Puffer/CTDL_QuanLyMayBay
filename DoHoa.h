...
//	int BLACK=0;
	...
	if(kbhit()) {
		...
		} else if(text.length()<min(68, limit) && ( u=='_' || u=='-' || (u>='0' && u<='9') || u == ' ')) {
			text.push_back(u);
			vanBan(x,y, text, BLACK, WHITE, 8, 2, 0, 1);
		}
	} else {
...
void khungNhapSo(int x, int y, string &text, int font, int charsize, int horiz, int vert, int limit) {
	static int temp=0;//bien co chuc nang trong viec tao dau nhap nhay
	temp=(temp+1)%20;
//	int WHITE=15;
//	int BLACK=0;
	...
	if(kbhit()) {
		...
		} else if(text.length()<min(68, limit) && (  u=='_' || u=='-' || (u>='0' && u<='9') || u == ' '||u=='.')) {
			text.push_back(u);
			vanBan(x,y, text, BLACK, WHITE, 8, 2, 0, 1);
		}
	} else {
